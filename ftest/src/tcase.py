...
from base import color_str, print_proc
...
class caseMgr:
	...
	"""
	def __init__(self, config_file, packet_file, script_dir):
	...
	def __load_case(self):
		...
		try:
			...
			for suit_dir, suit_file_list in p.items():
				for suit_file in suit_file_list:
	...
	def run(self, rep_mod, rep_dir, rep_success, silence):
		...
		sys.stdout.write(color_str("total %d case(s) testing... " % (total), "purple"))
		if silence:
			sys.stdout.flush()
			widgetType="percent"
			print_proc(cnt, total, widgetType=widgetType)
		else:
			print("")
		for cc in self.__case:
			...
			if not silence:
				...
				print(color_str("OK                       ", "green"))
				sys.stdout.write(color_str("generating report... ", "purple"))
			if not cc:
				print(color_str("failed", "red"))
				return
