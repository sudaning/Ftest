...
from base import color_str, language
...
class report(repComm):
	...
	def generation(self, mod, rep_success, silence):
		if not silence:
			sys.stdout.write("REPORT 01 " + "生成" + mod + "报告...")
		self.__mod = mod
		...
		if mod in ["xls"]:
			...
			f = self.__rep_xls()
		if not silence:
			print(color_str("OK", "green") + " 路径:" + f)

	def mail(self, conf, silence):
		if not silence:
			sys.stdout.write("REPORT 02 " + "发送报告邮件...")
		...
		if not smtp or not sender or not user or not password or not delivers:
			if not silence:
				print(color_str("MISSING CONFIG PARAMETER", "red"))
			return 

		if not enable:
			if not silence:
				print(color_str("DISABLE", "red"))
			return 
		...
		try:
			...
			server.quit()
			if not silence:
				print(color_str("OK", "green") + " 投递给 " + str(delivers.values()))
